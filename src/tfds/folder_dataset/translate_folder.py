...
class TranslateFolder(dataset_builder.DatasetBuilder):
  ...
  """
    ...
    def __init__(self, root_dir: str):
        ...
        self._split_examples, self._languages = _get_split_language_examples(root_dir)
        ...
        for split_name, examples in self._split_examples.items():
            split_dict.add(
                split_lib.SplitInfo(
                    name=split_name, shard_lengths=[len(next(iter(examples.values())))],
                )
            )
...
def _get_split_language_examples(root_dir: str) -> Tuple[SplitExampleDict, List[str]]:
  ...
  """
    ...
    for file in files:
        ...
        split_examples[split_name][lang] = _list_examples(os.path.join(root_dir, file))
        languages.add(lang)
    ...
    for split, examples in split_examples.items():
        ...
        if len(set(num_examples.values())) != 1:
            raise ValueError(
                "Num examples for split {} do not match: {}".format(split, num_examples)
            )
