...
class SplitInfo(object):
    ...
    def num_examples(self) -> int:
        if self.shard_lengths:  # pytype: disable=attribute-error
            return sum(
                int(sl) for sl in self.shard_lengths
    ...
    """
        ...
        return tfrecords_reader.make_file_instructions(
            name=self._dataset_name, split_infos=[self], instruction=str(self.name),
        )
...
class Split(str):
  ...
  * `TEST`: the testing data. This is the data to report metrics on. Typically
  ...
  """
    ...
    def __repr__(self) -> str:
        return "{}({})".format(
            type(self).__name__, super(Split, self).__repr__()
    ...
    def __getitem__(self, key):
        ...
        # 2nd case: Uses instructions: `info.splits['train[50%]']`
            ...
            instructions = tfrecords_reader.make_file_instructions(
                name=self._dataset_name, split_infos=self.values(), instruction=key,
            )
