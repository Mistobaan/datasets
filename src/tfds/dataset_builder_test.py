...
class DatasetBuilderTest(testing.TestCase):
    ...
    def test_determinism(self):
        ds = self.builder.as_dataset(split=splits_lib.Split.TRAIN, shuffle_files=False)
        ...
        self.assertEqual(
            l, [0, 17, 15, 1, 8, 5, 11, 3, 18, 16, 13, 2, 9, 19, 4, 12, 7, 10, 14, 6]
        )
...
class DatasetBuilderMultiDirTest(testing.TestCase):
    ...
    def test_default_multi_dir_old_version_exists(self):
        ...
        tf.io.gfile.makedirs(
            os.path.join(self.other_data_dir, "dummy_dataset_shared_generator", "0.1.0")
        )
        tf.io.gfile.makedirs(
            os.path.join(self.other_data_dir, "dummy_dataset_shared_generator", "0.2.0")
        )
        self.assertBuildDataDir(
            self.builder._build_data_dir(None), self.default_data_dir
        )
    ...
    def test_default_multi_dir_version_exists(self):
        ...
        constants.add_data_dir(self.other_data_dir)
        tf.io.gfile.makedirs(
            os.path.join(self.other_data_dir, "dummy_dataset_shared_generator", "1.0.0")
        )
        self.assertBuildDataDir(self.builder._build_data_dir(None), self.other_data_dir)
    ...
    def setUp(self):
        ...
        def load_mnist_dataset_info(self):
            mnist_info_path = os.path.join(
                utils.tfds_dir(), "testing/test_data/dataset_info/mnist/3.0.1",
            )
