...
class GetShardSpecsTest(testing.TestCase):
    ...
    @absltest.mock.patch.object(
        tfrecords_writer, "_get_number_shards", absltest.mock.Mock(return_value=6)
    )
    ...
    @absltest.mock.patch.object(
        tfrecords_writer, "_get_number_shards", absltest.mock.Mock(return_value=2)
    )
...
def _read_records(path):
  ...
  """
    ...
    all_recs = [
        list(dataset_utils.as_numpy(tf.data.TFRecordDataset(fpath))) for fpath in paths
    ]
...
class WriterTest(testing.TestCase):
    ...
    """
        ...
        self.assertEqual(
            all_recs, [[b"f", b"g"], [b"d"], [b"a", b"b"], [b"hi"], [b"e", b"c"],]
        )

    @absltest.mock.patch.object(example_parser, "ExampleParser", testing.DummyParser)
...
class TfrecordsWriterBeamTest(WriterTest):
    ...
    def _write(self, to_write, path, salt=""):
        ...
        with beam.Pipeline(options=options) as pipeline:
            ...
            _ = (
                pipeline | "test" >> _build_pcollection()
