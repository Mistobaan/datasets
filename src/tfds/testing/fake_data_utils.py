...
def get_random_audio(duration=_AUDIO_DURATION, sample=_SAMPLE_RATE):
    ...
    waveform = np.sin(2 * np.pi * sample_number * 440.0 / sample).astype(np.float32)
    waveform = waveform * 0.3
    return waveform


def get_random_wav_c1(channels=1, duration=_AUDIO_DURATION, sample=_SAMPLE_RATE):
...
def get_random_wav_c2(channels=2, duration=_AUDIO_DURATION, sample=_SAMPLE_RATE):
