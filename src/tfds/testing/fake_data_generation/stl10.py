...
from tfds.testing import test_utils

HEIGHT, WIDTH = (96, 96)
NUMBER_LABELS = 10

flags.DEFINE_string("tfds_dir", py_utils.tfds_dir(), "Path to tfds directory")
FLAGS = flags.FLAGS


def stl_output_dir():
    return os.path.join(
        FLAGS.tfds_dir, "testing", "test_data", "fake_examples", "stl10", "stl10_binary"
    )
...
def _generate_stl10_data():
    ...
    for fname in ["train_X.bin", "test_X.bin", "unlabeled_X.bin"]:
        images = np.random.randint(256, size=(1, HEIGHT * WIDTH * 3), dtype=np.uint8)
