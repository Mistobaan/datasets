...
def _read_files(
  ...
  """
    ...
    if read_config.input_context and read_config.input_context.num_input_pipelines > 1:
    ...
    if not read_config.input_context and hasattr(  # TODO(epot): Restore cardinality
...
class Reader(object):
  ...
  """
    ...
    def read(
        self, name, instructions, split_infos, read_config, shuffle_files,
    ):
    ...
    """
        ...
        def _read_instruction_to_ds(instruction):
            ...
            return self.read_files(
                file_instructions, read_config=read_config, shuffle_files=shuffle_files,
            )
    ...
    """
        if not file_instructions:
            msg = "Instruction {} corresponds to no data!".format(file_instructions)
...
class _RelativeInstruction(object):
    ...
    rounding = attr.ib(
        validator=attr.validators.in_(["closest", "pct1_dropremainder"])  # : str
    )
...
def _rel_to_abs_instr(rel_instr, name2len):
  ...
  """
    ...
    if split not in name2len:
        raise ValueError(
            'Unknown split "{}". Should be one of {}.'.format(split, list(name2len))
        )
...
class ReadInstruction(object):
  ...
  ds = tfds.load('mnist', split=tfds.ReadInstruction.from_spec(
      'test[:33%]'))
  ds = tfds.load('mnist', split=tfds.ReadInstruction(
      'test', to=33, unit='%'))
  ds = tfds.load('mnist', split=tfds.ReadInstruction(
  ...
  ds = tfds.load('mnist', split=(
      tfds.ReadInstruction('test', to=33, unit='%') +
      tfds.ReadInstruction('train', from_=1, to=-1, unit='abs')))
  ...
  """
    ...
    def __init__(
        self, split_name, rounding="closest", from_=None, to=None, unit=None,
    ):
