...
class Artifact(object):
    ...
    def __init__(self, name, url=None):
        ...
        self.url_info = checksums_lib.UrlInfo(
            size=len(content), checksum=_sha256(content),
        )
  ...
  """
    ...
    def test_download(self):
        ...
        downloads = manager.download(
            {"cached": a.url, "new": b.url, "info_deleted": c.url,}
        )
        expected = {
            "cached": a.file_path,
            "new": b.file_path,
    ...
    def test_extract(self):
        ...
        manager = self._get_manager()
        res = manager.extract({"cached": cached, "new": new_, "noextract": no_extract,})
        expected = {
    ...
    def test_download_and_extract(self):
        ...
        manager = self._get_manager(url_infos={a.url: a.url_info, b.url: b.url_info,})
        res = manager.download_and_extract({"a": a.url, "b": b.url})
        self.assertEqual(
            res, {"a": "/extract_dir/ZIP.%s" % a.file_name, "b": b.file_path,}
        )
    ...
    def test_download_and_extract_archive_ext_in_fname(self):
        ...
        manager = self._get_manager(url_infos={a.url: a.url_info,})
        res = manager.download_and_extract({"a": a.url})
        self.assertEqual(res, {"a": "/extract_dir/ZIP.%s" % a.file_name,})
    ...
    def test_force_download_and_extract(self):
        ...
        manager = self._get_manager(
            force_download=True, force_extraction=True, url_infos={a.url: a.url_info,}
        )
        res = manager.download_and_extract(a.url)
    ...
    def test_force_checksums_validation(self):
        ...
        dl_manager = self._get_manager(
            force_checksums_validation=True, register_checksums=False,
        )
    ...
    def test_download_cached(self):
        ...
        dl_manager = self._get_manager(
            register_checksums=False, url_infos={a.url: a.url_info},
        )
        self.assertEqual(dl_manager.download(a.url), a.url_path)
        self.assertCountEqual(self.downloaded_urls, [a.url])
        self.assertCountEqual(self.fs.files, [a.url_path, a.url_path + ".INFO"])
    ...
    def test_download_url_info_in_info_file_missmatch(self):
        ...
        self.dl_results[a.url] = checksums_lib.UrlInfo(
            size=a.url_info.size, checksum=_sha256("Other content"),
        )
        dl_manager = self._get_manager(register_checksums=False, force_download=True,)
