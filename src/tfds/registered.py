...
_FULL_NAME_REG = re.compile(
    r"^{ds_name}/({config_name}/)?{version}$".format(
        ds_name=r"\w+", config_name=r"[\w\-\.]+", version=r"[0-9]+\.[0-9]+\.[0-9]+",
    )
)
...
class RegisteredDataset(abc.ABCMeta):
    ...
    def __new__(cls, cls_name, bases, class_dict):
        ...
        builder_cls = super(
            RegisteredDataset, cls
        ).__new__(  # pylint: disable=too-many-function-args,redefined-outer-name
...
def builder_cls(name: str):
    """Fetches a `tfds.DatasetBuilder` class by string name.
...
def builder(name, **builder_init_kwargs):
  ...
  """
    ...
    with py_utils.try_reraise(prefix="Failed to construct dataset {}".format(name)):
  ...
  **Warning**: calling this function might potentially trigger the download
  ...
  Args:
    ...
    with_info: `bool`, if True, tfds.load will return the tuple
      (tf.data.Dataset, tfds.DatasetInfo) containing the info associated
      with the builder.
    builder_kwargs: `dict` (optional), keyword arguments to be passed to the
      `tfds.DatasetBuilder` constructor. `data_dir` will be passed
      through by default.
    download_and_prepare_kwargs: `dict` (optional) keyword arguments passed to
      `tfds.DatasetBuilder.download_and_prepare` if `download=True`. Allow
  ...
  Returns:
    ...
    ds_info: `tfds.DatasetInfo`, if `with_info` is True, then `tfds.load`
...
def _iter_full_names(
    predicate_fn: Optional[PredicateFn], current_version_only: bool,
) -> Iterator[str]:
    """Yield all registered datasets full_names (see `list_full_names`)."""
    for (
        builder_name,
        builder_cls,
    ) in _DATASET_REGISTRY.items():  # pylint: disable=redefined-outer-name
        ...
        for full_name in _iter_single_full_names(
            builder_name, builder_cls, current_version_only=current_version_only,
        ):
            yield full_name


def list_full_names(
    predicate_fn: Optional[PredicateFn] = None, current_version_only: bool = False,
) -> List[str]:
  ...
  """
    return sorted(
        _iter_full_names(
            predicate_fn=predicate_fn, current_version_only=current_version_only,
        )
    )


def single_full_names(
    builder_name: str, current_version_only: bool = True,
) -> List[str]:
