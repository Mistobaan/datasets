...
def _get_shard_specs(
    num_examples: int, total_size: int, bucket_lengths: List[int], path: str,
) -> List[_ShardSpec]:
...
def _write_tfrecord(
    path: str, iterator: Iterable[bytes],
):
  ...
  """
    ...
    def finalize(self):
        ...
        try:
            ...
            _raise_error_for_duplicated_keys(err.item1, err.item2, self._example_specs)
  ...
  """
    ...
    """
        self._original_state = dict(
            example_specs=example_specs, path=path, hash_salt=hash_salt
        )
        self._path = path
    ...
    def _serialize_shard(
        self, key_example: Tuple[hashing.HashKey, Example],
        ...
        for i in range(len(examples) - 1):
            if examples[i][0] == examples[i + 1][0]:
                _raise_error_for_duplicated_keys(
                    examples[i][1], examples[i + 1][1], self._example_specs
                )
        examples = [ex[1] for ex in examples]
        total_size = sum(len(ex) for ex in examples)
        yield beam.pvalue.TaggedOutput(
            self._OUTPUT_TAG_BUCKETS_LEN_SIZE, (bucketid, (len(examples), total_size))
        )
    ...
    def _write_final_shard(self, shardid_examples):
        ...
        examples = list(itertools.chain(*[ex[1] for ex in sorted(examples_by_bucket)]))
