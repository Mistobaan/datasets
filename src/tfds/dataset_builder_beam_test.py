    ...
    def _split_generators(self, dl_manager):
        ...
        return [
            splits_lib.SplitGenerator(
                name=splits_lib.Split.TRAIN, gen_kwargs=dict(num_examples=1000),
            ),
            splits_lib.SplitGenerator(
                name=splits_lib.Split.TEST, gen_kwargs=dict(num_examples=725),
            ),
        ]
...
class BeamBasedBuilderTest(testing.TestCase):
    ...
    def test_download_prepare(self):
        ...
        if not dl_config:
            return
        self._assertBeamGeneration(dl_config, DummyBeamDataset, "dummy_beam_dataset")
