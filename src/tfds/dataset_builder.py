  ...
  **Configuration**: Some `DatasetBuilder`s expose multiple variants of the
  ...
  """
    ...
    """
        ...
        if not self._builder_config and not self.VERSION:
            raise AssertionError(
                ...
                "`VERSION = tfds.Version('x.y.z')` to the class.".format(self.name)
            )
    ...
    def _build_single_dataset(
        self, split, shuffle_files, batch_size, decoders, read_config, as_supervised,
    ):
        ...
        if (
            shuffle_files
            and read_config.options.experimental_deterministic is None
    ...
    @abc.abstractmethod
    def _as_dataset(self, split, decoders=None, read_config=None, shuffle_files=False):
        """Constructs a `tf.data.Dataset`.
...
class BeamBasedBuilder(FileAdapterBuilder):
    ...
    def _make_split_generators_kwargs(self, prepare_split_kwargs):
        ...
        split_generators_arg_names = (
            inspect.getargspec(self._split_generators).args
            if six.PY2
            else inspect.signature(  # pylint: disable=deprecated-method  # pytype: disable=wrong-arg-types
    ...
    def _download_and_prepare(self, dl_manager, download_config):
        ...
        beam_options = (
            ...
            or beam.options.pipeline_options.PipelineOptions()
        )
        ...
        with beam.Pipeline(
            runner=download_config.beam_runner, options=beam_options,
        ) as pipeline:
